"""Character Restock Calculator – Personal hangar restock list management and gap analysis (Phase 4C)."""
...
router = APIRouter(prefix="/api/character-restock", tags=["character-restock"])


async def _assert_list_owned(db: AsyncSession, user_id: int, rl) -> None:
    """Raise 403 unless the account owns the list's character."""
    await assert_owns_character(db, user_id, rl.character_id)
...
async def _calculate_item_gap(
    ...
    """Calculate current stock, gap, and to_buy for a single type_id in character hangars.

    current_stock = SUM(quantity) from character assets where type_id matches.
    gap = max(0, target_quantity - current_stock)
    to_buy = gap
    """
    # Sum quantity of this type_id in character personal assets
...
async def get_restock_lists(
    ...
    """Get all character restock lists with optional filters."""
    owned_ids = await get_owned_character_ids(db, user_id)
    base = select(CharacterRestockList)

    if character_id:
        await assert_owns_character(db, user_id, character_id)
        base = base.where(CharacterRestockList.character_id == character_id)
    else:
        base = base.where(CharacterRestockList.character_id.in_(owned_ids or [0]))
    if is_active is not None:
        base = base.where(CharacterRestockList.is_active == is_active)
    ...
    query = base.order_by(CharacterRestockList.name).offset(offset).limit(per_page)
    ...
    for rl in lists:
        item_count_query = select(func.count()).where(
            CharacterRestockListItem.restock_list_id == rl.id
        )
        item_count = await db.scalar(item_count_query) or 0
        lists_data.append({
            "id": rl.id,
            "character_id": rl.character_id,
            "name": rl.name,
...
async def create_restock_list(
    ...
    """Create a new character restock list."""
    await assert_owns_character(db, user_id, character_id)
    rl = CharacterRestockList(
...
async def get_restock_list(
    ...
    """Get a single character restock list with its items."""
    stmt = select(CharacterRestockList).where(CharacterRestockList.id == list_id)
    ...
    # Fetch items
    items_stmt = select(CharacterRestockListItem).where(
        CharacterRestockListItem.restock_list_id == list_id
    ).order_by(CharacterRestockListItem.category_group, CharacterRestockListItem.type_name)
...
async def update_restock_list(
    ...
    """Update a character restock list's name or active status."""
    stmt = select(CharacterRestockList).where(CharacterRestockList.id == list_id)
    result = await db.execute(stmt)
    rl = result.scalar_one_or_none()
...
async def delete_restock_list(
    ...
    """Delete a character restock list and all its items."""
    stmt = select(CharacterRestockList).where(CharacterRestockList.id == list_id)
    result = await db.execute(stmt)
    rl = result.scalar_one_or_none()
    ...
    del_items = delete(CharacterRestockListItem).where(
        CharacterRestockListItem.restock_list_id == list_id
    )
...
async def add_restock_item(
    ...
    """Add an item to a character restock list with a target quantity."""
    # Verify list exists
    stmt = select(CharacterRestockList).where(CharacterRestockList.id == list_id)
    result = await db.execute(stmt)
    rl = result.scalar_one_or_none()
    ...
    dup_stmt = select(CharacterRestockListItem).where(
        CharacterRestockListItem.restock_list_id == list_id,
        CharacterRestockListItem.type_id == type_id,
    )
    ...
    gap_data = await _calculate_item_gap(
        db, rl.character_id, type_id, target_quantity
    )

    item = CharacterRestockListItem(
...
async def update_restock_item(
    ...
    stmt = select(CharacterRestockListItem).where(
        CharacterRestockListItem.id == item_id,
        CharacterRestockListItem.restock_list_id == list_id,
    )
    result = await db.execute(stmt)
    ...
    # Get character_id from the parent list
    list_stmt = select(CharacterRestockList).where(CharacterRestockList.id == list_id)
    ...
    if target_quantity is not None and rl:
        gap_data = await _calculate_item_gap(
            db, rl.character_id, item.type_id, item.target_quantity
        )
...
async def delete_restock_item(
    ...
    """Remove an item from a character restock list."""
...
async def add_template_to_list(
    ...
    """Add a predefined category template to a character restock list."""
    if template not in CHARACTER_RESTOCK_CATEGORIES:
        valid = ", ".join(CHARACTER_RESTOCK_CATEGORIES.keys())
        raise HTTPException(
            status_code=400,
    ...
    added = 0
    skipped = 0
    for type_id, type_name in CHARACTER_RESTOCK_CATEGORIES[template]:
        # Check for duplicate
        dup_stmt = select(CharacterRestockListItem).where(
            CharacterRestockListItem.restock_list_id == list_id,
            CharacterRestockListItem.type_id == type_id,
...
async def get_buy_text(
    ...
    items_stmt = select(CharacterRestockListItem).where(
        CharacterRestockListItem.restock_list_id == list_id,
        CharacterRestockListItem.to_buy > 0,
    ).order_by(CharacterRestockListItem.category_group, CharacterRestockListItem.type_name)
    items_result = await db.execute(items_stmt)
    items = items_result.scalars().all()
