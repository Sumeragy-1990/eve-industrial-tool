...
(function () {
    ...
    function exportBuyOrder() {
        ...
        async function generateBuyList() {
            ...
            // 2. Auch Sub-Sub-Orders durchgehen (Rekursion für tiefere Ebenen)
            var _subSubs = _productionOrders.filter(function(o) {
                var _p = _productionOrders.find(function(p) { return p.id === o.parent_id; });
                return _p && _p.type === 'sub';
            });
            for (var _ssi = 0; _ssi < _subSubs.length; _ssi++) {
                var _sso = _subSubs[_ssi];
