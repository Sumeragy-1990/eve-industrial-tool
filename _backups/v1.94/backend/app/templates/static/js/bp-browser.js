...
(function () {
    ...
    async function selectBlueprintSubMode(productTypeId, mode) {
        ...
        if (mode === "custom") {
            ...
            // Fallback: defaults basierend auf meta_group (T2=2/4, Faction=0/0, sonst 10/20)
            const foundProduct = findProductInTree(productTypeId);
            var _defMe = 10, _defTe = 20;
            if (foundProduct) {
                var mg = (foundProduct.meta_group_name || '').toLowerCase();
                if (mg.indexOf('tech ii') >= 0 || mg.indexOf('t2') >= 0 || mg.indexOf('storyline') >= 0) {
                    _defMe = 2; _defTe = 4;
                } else if (mg.indexOf('faction') >= 0 || mg.indexOf('officer') >= 0 || mg.indexOf('deadspace') >= 0) {
                    _defMe = 0; _defTe = 0;
                }
            }
            if (mode === "bpo") {
                ...
                if (foundProduct && foundProduct.best_me != null) _defMe = foundProduct.best_me;
                if (foundProduct && foundProduct.best_te != null) _defTe = foundProduct.best_te;
            } else if (mode === "bpc") {
                const modeEl = document.getElementById("bpDetailSubMode");
            ...
            const meSlider = document.getElementById("bpConfigMe");
            const meVal = document.getElementById("bpConfigMeValue");
            if (meSlider) { meSlider.value = _defMe; if (meVal) meVal.textContent = String(_defMe); }
            const teSlider = document.getElementById("bpConfigTe");
            const teVal = document.getElementById("bpConfigTeValue");
            if (teSlider) { teSlider.value = _defTe; if (teVal) teVal.textContent = String(_defTe); }
        }
