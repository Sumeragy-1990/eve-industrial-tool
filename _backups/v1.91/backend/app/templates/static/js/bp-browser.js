...
(function () {
    ...
    async function selectBlueprintSubMode(productTypeId, mode) {
        ...
        if (mode === "custom") {
            ...
            if (meSlider) { meSlider.value = 2; if (meVal) meVal.textContent = "2"; }
            const teSlider = document.getElementById("bpConfigTe");
            const teVal = document.getElementById("bpConfigTeValue");
            if (teSlider) { teSlider.value = 4; if (teVal) teVal.textContent = "4"; }
            const runsInput = document.getElementById("bpConfigRuns");
    ...
    async function loadProductDetail(blueprintTypeId) {
        const me = parseInt(document.getElementById("bpConfigMe").value) || 2;
        const te = parseInt(document.getElementById("bpConfigTe").value) || 4;
    ...
    function _renderBuildStepNode(step, depth) {
        ...
        if (!isBuy && !isReaction && step.blueprint_type_id) {
            ...
            var _bstMe = _bstOvr.me != null ? _bstOvr.me : (step.me != null ? step.me : 2);
            var _bstTe = _bstOvr.te != null ? _bstOvr.te : (step.te != null ? step.te : 4);
        ...
        if (!item._buildStepsData) {
            ...
            try {
                ...
                if (!bpid) {
                    ...
                    var itemMe = item.me != null ? item.me : 2;
                    var itemTe = item.te != null ? item.te : 4;
            ...
            if (resp.ok) {
                ...
                for (var _sii = 0; _sii < order.items.length; _sii++) {
                    ...
                    if (_siItem._buildStepsData) {
                        ...
                        _createSubOrdersRecursive(_siItem._buildStepsData, order.id, _siSubComps, _siItem.me||2, _siItem.te||4, order.config, 1);
                    }
                }
