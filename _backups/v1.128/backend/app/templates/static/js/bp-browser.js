...
(function () {
            ...
            if (resp.ok) {
                ...
                for (var _sii = 0; _sii < order.items.length; _sii++) {
                    ...
                    if (_siItem._buildStepsData) {
                        ...
                        _createSubOrdersRecursive(_siItem._buildStepsData, order.id, _siSubComps, _siItem.me||10, _siItem.te||20, order.config, 1);
                    }
                }
    ...
    function renderOrderSummary() {
        ...
        // Grand Total — API-Wert bevorzugen, sonst recalculated
        var grandTotal = (order.build_cost && order.build_cost.total_cost > 0)
            ? order.build_cost.total_cost
            : (totalMaterialCost + totalJobCost + totalBpcCost);
        // Falls build_cost nur für das erste Item gilt (Single-Item Order)
        if (grandTotal <= 0 && order.items.length === 1 && order.items[0].build_cost && order.items[0].build_cost.total_cost > 0) {
            grandTotal = order.items[0].build_cost.total_cost;
        }
    ...
    function exportBuyOrder() {
        ...
        async function generateBuyList() {
            ...
            // Aggregate buy materials from MAIN order + all SUB orders
            const buyMap = {}; // material_type_id -> { name, qty, total_cost }
            let hasCostData = false;

            // Collect all orders: main + its sub-orders (recursively)
            var _allBuyOrders = [order];
            var _subs = _productionOrders.filter(function(o) { return o.parent_id === order.id; });
            for (var _si2 = 0; _si2 < _subs.length; _si2++) _allBuyOrders.push(_subs[_si2]);

            for (var _oi = 0; _oi < _allBuyOrders.length; _oi++) {
                var _ao = _allBuyOrders[_oi];
                for (var _ii = 0; _ii < (_ao.items || []).length; _ii++) {
                    var _item = _ao.items[_ii];
                    if (!_item.materials) continue;
                    for (var _mi2 = 0; _mi2 < _item.materials.length; _mi2++) {
                        var _m = _item.materials[_mi2];
                        if (_m.decision !== 'buy') continue;
                        ...
                        if (!buyMap[key]) {
                            buyMap[key] = {
                                ...
                                type_id: key,
                                qty: 0,
                                unit_price: _m.unit_price || 0,
                                total_cost: 0
                            };
                        }
                        buyMap[key].qty += _m.total_quantity;
                        var cost = (_m.unit_price || 0) * _m.total_quantity;
                        buyMap[key].total_cost += cost;
                        if (_m.unit_price) hasCostData = true;
                    }
                }
            }
