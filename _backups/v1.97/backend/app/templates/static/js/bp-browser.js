        ...
        for (let i = 0; i < order.items.length; i++) {
            ...
            if (expanded && hasMaterials) {
                ...
                for (let mi = 0; mi < item.materials.length; mi++) {
                    ...
                    // Sell price per unit
                    var sellPrice = m.sell_price_per_unit;
                    if (!sellPrice) { var _ce = getPrice(m.material_type_id); if (_ce) sellPrice = _ce.sell_price_min; }
                    if (!sellPrice) { var _fi = getEffectivePrice(m.material_type_id); if (_fi.price) sellPrice = _fi.price; }
                    if (!sellPrice && m.unit_price) sellPrice = m.unit_price;
                    // Buy price per unit
                    var buyPrice = m.buy_price_per_unit;
                    if (!buyPrice) { var _ce2 = getPrice(m.material_type_id); if (_ce2) buyPrice = _ce2.buy_price_max; }
                    if (!buyPrice) { var _fi2 = getEffectivePrice(m.material_type_id); if (_fi2.price) buyPrice = _fi2.price; }
                    if (!buyPrice && m.unit_price) buyPrice = m.unit_price;
