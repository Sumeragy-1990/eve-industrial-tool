    ...
    for item in body.cart_items:
        ...
        for m in mats:
            ...
            if m_is_reaction:
                ...
                # EVE-korrekte Formel: ceil(base + base * wasteFactor/(1+ME) * (1-bonus))
                total_bonus = (getattr(item, '_rig_bonus', 0.0) or 0.0)
                raw_waste = base_qty * 0.1 / (1.0 + item.me)
                effective_waste = raw_waste * max(0.0, 1.0 - total_bonus)
                adjusted = max(1, math.ceil(base_qty + effective_waste))
    ...
    async def resolve_step(
        ...
        for row in rows:
            ...
            if step_is_reaction:
                ...
                if base_qty > 0:
                    total_bonus = (getattr(item, '_rig_bonus', 0.0) or 0.0)
                    raw_waste = base_qty * 0.1 / (1.0 + step_me)
                    effective_waste = raw_waste * max(0.0, 1.0 - total_bonus)
                    adjusted = max(1, math.ceil(base_qty + effective_waste))
    ...
    async def _fetch_materials(bp_type_id: int, me_level: int, te_level: int,
        ...
        for row in rows:
            ...
            if is_reaction:
                ...
                if base_qty > 0:
                    total_bonus = (getattr(item, '_rig_bonus', 0.0) or 0.0)
                    raw_waste = base_qty * 0.1 / (1.0 + me_level)
                    effective_waste = raw_waste * max(0.0, 1.0 - total_bonus)
                    adjusted = max(1, math.ceil(base_qty + effective_waste))
