...
(function () {
    ...
    /** Debug: pricing diagnostic — logs all sub-order vs parent build prices */
    function debugPricingDiagnostic() {
        var lines = [];
        lines.push("=== PRICING DIAGNOSTIC ===");
        lines.push("Active Order Index: " + _activeOrderIndex);
        for (var i = 0; i < _productionOrders.length; i++) {
            var o = _productionOrders[i];
            if (o.type === 'sub' && o.parent_id) {
                ...
                var parentBuildPrice = null;
                if (parentOrder) {
                    for (var pi = 0; pi < parentOrder.items.length; pi++) {
                        var pItem = parentOrder.items[pi];
                        if (!pItem.materials) continue;
                        for (var mi = 0; mi < pItem.materials.length; mi++) {
                            var m = pItem.materials[mi];
                            if (m.material_type_id === matId) {
                                parentBuildPrice = m._subOrderTotal || m.total_cost || null;
                                break;
                            }
                        }
                    }
                }
                var match = (subTotal === parentBuildPrice) ? "✓ MATCH" : "✗ MISMATCH";
                lines.push("---");
                lines.push("Sub-Order: " + (o.name || '?') + " (id=" + o.id + ")");
                lines.push("  Sub Total: " + (subTotal ? formatIsk(subTotal) : 'N/A'));
                lines.push("  Parent Build: " + (parentBuildPrice ? formatIsk(parentBuildPrice) : 'N/A'));
                lines.push("  " + match);
                if (parentBuildPrice && subTotal && Math.abs(subTotal - parentBuildPrice) > 1) {
                    lines.push("  Diff: " + formatIsk(Math.abs(subTotal - parentBuildPrice)));
                }
                if (parentOrder) {
                    lines.push("  Config SCI: " + (o.config && o.config.system_cost_index));
                    lines.push("  Config Tax: " + (o.config && o.config.tax_rate));
                    lines.push("  Config Facility: " + (o.config && o.config.facility_type));
                    lines.push("  Runs: " + (o.items && o.items[0] ? o.items[0].runs : '?'));
                    lines.push("  ME: " + (o.items && o.items[0] ? o.items[0].me : '?'));
                    lines.push("  TE: " + (o.items && o.items[0] ? o.items[0].te : '?'));
                }
            }
        }
        lines.push("=== END ===");
        ...
        modal.style.cssText = "position:fixed;top:10%;left:10%;width:80%;height:70%;z-index:9999;background:#1a1a2e;border:1px solid rgba(255,255,255,0.2);border-radius:8px;padding:16px;overflow:auto;box-shadow:0 8px 32px rgba(0,0,0,0.7);";
        modal.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:8px;">' +
            '<h5 style="margin:0;color:#ffc107;">Pricing Diagnostic</h5>' +
            '<button onclick="this.parentElement.parentElement.remove()" style="background:none;border:none;color:#888;font-size:1.2rem;cursor:pointer;">✕</button></div>' +
            '<pre style="font-size:0.65rem;color:#ccc;white-space:pre-wrap;margin:0;">' + escHtml(text) + '</pre>' +
            '<div style="margin-top:8px;"><button onclick="navigator.clipboard.writeText(document.querySelector(\'#bpDiagModal pre\').textContent);this.textContent=\'✅ Copied!\'" ' +
            'style="padding:4px 12px;background:#0d6efd;color:#fff;border:none;border-radius:4px;cursor:pointer;">📋 Copy</button></div>';
