...
async def require_auth_optional(request: Request) -> Optional[int]:
    """Optional auth: returns character_id or None (doesn't raise 401)."""
    char_id = request.session.get("character_id")
    user_id = request.session.get("user_id")
    if not char_id or not user_id:
        return None
