...
async def calculate_build_cost(
    ...
    _user: Optional[int] = Depends(require_auth_optional),
    db: AsyncSession = Depends(get_session),
):
    """
    ...
    _RIG_FALLBACK = {"none": 0.0, "t1": 0.02, "t2": 0.024,
        ...
        # EVE T1 Manufacturing Rigs: 2% pro Rig
        "m_basic_large_ship_mat_1": 0.02,
        "m_basic_medium_ship_mat_1": 0.02,
        "m_basic_small_ship_mat_1": 0.02,
        "m_basic_capital_ship_mat_1": 0.02,
        "m_basic_adv_component_mat_1": 0.02,
        "m_basic_component_mat_1": 0.02,
        # T2 Manufacturing Rigs: 2.4%
        "m_rig_large_ship_mat_1": 0.024,
        "m_rig_medium_ship_mat_1": 0.024,
        "m_rig_small_ship_mat_1": 0.024,
        "m_rig_capital_ship_mat_1": 0.024,
        "m_rig_adv_component_mat_1": 0.024,
        "m_rig_component_mat_1": 0.024,
    }
    ...
    # Structure Role Material Bonus (Engineering Complex Bonus)
    structure_mat_bonus = {"raitaru": 0.01, "sotyo": 0.02, "azbel": 0.03}.get(facility.facility_type, 0.0)

    # ── Per-Item Rig Categorization ──
    # Bestimme pro Item (Blueprint) welches Rig greift.
    # Rig-Namen enthalten die Kategorie (_large_ship_, _medium_ship_, _adv_component_).
    # Wir prüfen die product_type_id in sde_items.
    # Simple Mapping: Rig-Name-Teil → product.category_id
    # NOTE: ship_size fehlt in SDE-Import (attribute_id 633 nicht vorhanden).
    # Stattdessen: group_id-basierte Größenschätzung.
    # Bekannte Ship-Gruppen: 25=Battleship(L), 26=Cruiser(M), 29=Frigate(S),
    #   419=Battlecruiser(M), 420=Destroyer(S), 547=Dreadnought(Cap)
    _SHIP_SIZE_GROUPS = {"large": [25, 28, 30], "medium": [26, 419], "small": [29, 420], "capital": [31, 485, 547, 883, 1026]}
    _RIG_CAT_MAP = {
        "ship": [6],            # category_id 6 = Ship
        "adv_component": [873], # Advanced Component
        "component": [7],       # Component
    }
    # Hilfsfunktion: Prüft ob ein Rig-Name auf die Kategorie + Größe passt
    def _rig_matches(rig_name, prod_cat, prod_group=None):
        rl = rig_name.lower()
        # Kategorie-Check
        for key, cats in _RIG_CAT_MAP.items():
            if key in rl and prod_cat in cats:
                if prod_cat == 6 and prod_group is not None:
                    # Ship: prüfe Größe
                    for size, groups in _SHIP_SIZE_GROUPS.items():
                        if size in rl and prod_group in groups:
                            return True
                    if not any(s in rl for s in ["large","medium","small","capital"]):
                        return True  # Fallback: kein Größen-Keyword -> alle Ships
                    return False  # Größe passt nicht
                return True
        return False
    # Lookup product category + group for each item
    for item in body.cart_items:
        prod_cat = None
        prod_group = None
        if item.product_type_id:
            cat_sql = text("SELECT category_id, group_id FROM sde_items WHERE type_id = :tid")
            cat_res = await db.execute(cat_sql, {"tid": item.product_type_id})
            cat_row = cat_res.fetchone()
            if cat_row:
                prod_cat = cat_row[0]
                prod_group = cat_row[1]
        # Filter rigs that match this item's category + size
        matched_bonus = 0.0
        for r in rig_names:
            r_clean = r.strip()
            rig_bonus = rig_mat_db.get(r_clean, _RIG_FALLBACK.get(r_clean, 0.0))
            if rig_bonus <= 0:
                continue
            if prod_cat is None or _rig_matches(r_clean, prod_cat, prod_group):
                matched_bonus += rig_bonus
        # Cap per item - separate factors für multiplicative Anwendung
        matched_bonus = min(matched_bonus, 0.10) * _SEC_MULT.get(facility.security_class, 1.0)
        item._rig_part = 1.0 - matched_bonus
        item._struct_part = 1.0 - structure_mat_bonus
        item._rig_bonus = min(matched_bonus + structure_mat_bonus, 0.15)
    rig_mat_bonus = 0.0  # Wird pro Item gesetzt
    ...
    for item in body.cart_items:
        ...
        for m in mats:
            ...
            if m_is_reaction:
                ...
                # EVE-Formel: multiplier = (1 - ME/100) * (1 - rig) * (1 - structure)
                # Material Efficiency -10.0% bei ME=10 bedeutet 10% Reduktion
                total_bonus = (getattr(item, '_rig_bonus', 0.0) or 0.0)
                me_factor = 1.0 - (item.me / 100.0)
                # Rig+Structure sind MULTIPLIKATIV, nicht additiv!
                # Bei 2% Rig + 1% Structure: 0.98 * 0.99 = 0.9702, NICHT 1 - 0.03 = 0.97
                rig_part = getattr(item, '_rig_part', 1.0)
                struct_part = getattr(item, '_struct_part', 1.0)
                bonus_factor = rig_part * struct_part
                multiplier = max(0.01, me_factor * bonus_factor)
                adjusted = max(1, math.ceil(base_qty * multiplier))
                if base_qty > 1000:  # Debug für große Mengen
                    print(f"[DBG] mat={m.material_type_id} base={base_qty} me={item.me} rig_part={rig_part:.4f} struct_part={struct_part:.4f} mult={multiplier:.6f} -> adj={adjusted}")
