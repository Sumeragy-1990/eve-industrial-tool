...
(function () {
    ...
    async function selectBlueprintSubMode(productTypeId, mode) {
        ...
        if (mode === "custom") {
            ...
            const foundProduct = findProductInTree(productTypeId);
            // Defaults by meta_group_id:
            //   1 (T1) → ME 10 / TE 20 | 2 (T2) → ME 2 / TE 4 | 3+ (Faction etc.) → ME 0 / TE 0
            var _mg = foundProduct ? foundProduct.meta_group_id : 1;
            var _mgName = foundProduct ? (foundProduct.meta_group_name || '').toLowerCase() : '';
            var defaultMe = 10, defaultTe = 20;
            if (_mg === 2 || _mgName.indexOf('tech ii') >= 0) { defaultMe = 2; defaultTe = 4; }
            else if (_mg !== 1 || _mgName.indexOf('faction') >= 0 || _mgName.indexOf('deadspace') >= 0 || _mgName.indexOf('officer') >= 0) { defaultMe = 0; defaultTe = 0; }
            const meSlider = document.getElementById("bpConfigMe");
            const meVal = document.getElementById("bpConfigMeValue");
            if (meSlider) { meSlider.value = defaultMe; if (meVal) meVal.textContent = String(defaultMe); }
            const teSlider = document.getElementById("bpConfigTe");
            const teVal = document.getElementById("bpConfigTeValue");
            if (teSlider) { teSlider.value = defaultTe; if (teVal) teVal.textContent = String(defaultTe); }
            const runsInput = document.getElementById("bpConfigRuns");
        ...
        if (!item._buildStepsData) {
            ...
            try {
                ...
                if (!bpid) {
                    ...
                    var resp = await fetch("/api/blueprints/" + bpid + "/build-steps?me=" + itemMe + "&te=" + itemTe + "&runs=" + itemRuns + "&max_depth=5");
                    if (resp.ok) {
                        var data = await resp.json();
    ...
    function _diagnosticCompare() {
        console.log('=== PRICE DIAGNOSTIC ===');
        var _main = _productionOrders.find(function(o) { return o.type !== 'sub'; });
        if (!_main) { console.log('No main order found'); return; }
        var _subs = _productionOrders.filter(function(o) { return o.type === 'sub' && o.parent_id === _main.id; });
        console.log('Main:', _main.name, '- Sub orders:', _subs.length);
        for (var _si = 0; _si < _subs.length; _si++) {
            var _so = _subs[_si];
            var _subItem = _so.items && _so.items[0];
            var _soTotal = (_subItem && _subItem.build_cost) ? _subItem.build_cost.total_cost : 0;
            var _typeId = _so._parentMatTypeId || (_subItem ? _subItem.product_type_id : null);
            var _matched = null;
            for (var _mi = 0; _mi < (_main.items || []).length; _mi++) {
                var _item = _main.items[_mi];
                for (var _mm = 0; _mm < (_item.materials || []).length; _mm++) {
                    if (_item.materials[_mm].material_type_id === _typeId) {
                        _matched = _item.materials[_mm];
                        break;
                    }
                }
                if (_matched) break;
            }
            var _mainTotal = _matched ? (_matched._subOrderTotal || _matched.total_cost || 0) : 0;
            var _diff = Math.abs(_soTotal - _mainTotal);
            console.log('Sub ' + (_si+1) + ': ' + (_subItem ? _subItem.product_name : '?') + ' (' + _typeId + ')' +
                ' ORDER total=' + _soTotal + ' MAIN total=' + _mainTotal + ' diff=' + _diff +
                (_diff < 1000 ? ' ✓ MATCH' : ' ✗ MISMATCH'));
        }
    ...
    async function setActiveOrder(index) {
        ...
        if (order && order.items) {
            ...
            // Nur bei neuer Order (kein build_cost) refreshen — sonst bleiben Werte stabil
            if (!order.items[0] || !order.items[0].build_cost || !order.items[0].build_cost.total_cost) {
    ...
    async function _fetchBuildCostsForOrder(order) {
        ...
        // Resolved config auf Order speichern, damit Sub-Orders sie erben
        ...
        try {
            ...
            if (resp.ok) {
                ...
                for (const apiItem of data.items) {
                    ...
                    // Apply override prices to materials (überschreibt API-Werte)
                    if (orderItem.materials) {
                        for (var _omi = 0; _omi < orderItem.materials.length; _omi++) {
                            var _om = orderItem.materials[_omi];
                            var _omEff = getEffectivePrice(_om.material_type_id);
                            if (_omEff && _omEff.source === 'override' && _omEff.price != null) {
                                _om.unit_price = _omEff.price;
                                _om.total_cost = _omEff.price * (_om.total_quantity || 0);
                                _om.sell_price_per_unit = _omEff.price;
                                _om.buy_price_per_unit = _omEff.price;
                            }
                        }
                        var _overrideMatTotal = 0;
                        for (var _omi2 = 0; _omi2 < orderItem.materials.length; _omi2++) {
                            _overrideMatTotal += orderItem.materials[_omi2].total_cost || 0;
                        }
                        if (orderItem.build_cost) {
                            var _oldJob = (orderItem.build_cost.total_job_cost || 0) + (orderItem.build_cost.installation_cost || 0);
                            orderItem.build_cost.total_material_cost = _overrideMatTotal;
                            orderItem.build_cost.total_cost = _overrideMatTotal + _oldJob;
                            orderItem.build_cost.cost_per_unit = orderItem.build_cost.total_cost / Math.max(orderItem.runs || 1, 1);
                        }
                    }
                }
                ...
                for (const orderItem of order.items) {
                    ...
                    try {
                        ...
                        var _bsMe = orderItem.me != null ? orderItem.me : 2;
                        var _bsTe = orderItem.te != null ? orderItem.te : 4;
                    const bstepsResp = await fetch(
                            "/api/blueprints/" + orderItem.blueprint_type_id + "/build-steps?runs=" + _bsRuns + "&me=" + _bsMe + "&te=" + _bsTe + "&max_depth=5",
                            { credentials: "include" }
                        );
                        if (bstepsResp.ok) {
                ...
                // User-Änderungen an alten Sub-Orders sichern (ME/TE)
                var _savedSubData = {};
                for (var _spi = 0; _spi < _productionOrders.length; _spi++) {
                    var _spo = _productionOrders[_spi];
                    if (_spo.parent_id === _mainOrderId && _spo.items && _spo.items[0]) {
                        var _sk = _spo._parentMatTypeId || _spo.items[0].product_type_id;
                        _savedSubData[_sk] = {
                            me: _spo.items[0].me,
                            te: _spo.items[0].te,
                            decision: _spo.items[0].decision,
                            config: _spo.config ? JSON.parse(JSON.stringify(_spo.config)) : null,
                            materials: _spo.items[0].materials ? _spo.items[0].materials.map(function(m) {
                                return { material_type_id: m.material_type_id, decision: m.decision, _priceMode: m._priceMode };
                            }) : null
                        };
                    }
                }
                ...
                function _createSubOrdersRecursive(buildSteps, parentOrderId, subComponents, meLevel, teLevel, parentConfig, depth) {
                    ...
                    for (var _rsi = 0; _rsi < buildSteps.steps.length; _rsi++) {
                        ...
                        for (var _rss = 0; _rss < _step.sub_steps.length; _rss++) {
                            ...
                            _subCnt++;
                            console.log("[BP] SubOrder depth="+depth+" cnt="+_subCnt+" name="+(_subS.product_name||'?')+" mats="+(_subS.materials?1:0)+" deeper="+((_subS.sub_steps||[]).length));
                            var _rpid = _subS.product_type_id;
                            var _rbpId = _subS.blueprint_type_id || _rpid;
                            var _matchedSC = null;
                            if (subComponents) {
                                for (var _rc2 = 0; _rc2 < subComponents.length; _rc2++) {
                                    if (subComponents[_rc2].product_type_id === _rpid || subComponents[_rc2].blueprint_type_id === _rbpId) {
                                        _matchedSC = subComponents[_rc2];
                                        break;
                                    }
                                }
                            }
                            var _subMatCost = 0;
                            if (!_matchedSC || !_matchedSC.costs || !_matchedSC.costs.step_total) {
                                for (var _mi2 = 0; _mi2 < (_subS.materials || []).length; _mi2++) {
                                    var _sm2 = _subS.materials[_mi2];
                                    var _pr2 = getEffectivePrice(_sm2.material_type_id);
                                    _subMatCost += (_pr2 && _pr2.price) ? _pr2.price * (_sm2.total_quantity || 0) : 0;
                                }
                            }
                            var _scCosts = _matchedSC && _matchedSC.costs;
                            var _subBC = {
                                total_material_cost: (_scCosts && _scCosts.material_cost) || _subMatCost || 0,
                                system_cost_amount: (_scCosts && _scCosts.job_cost) || 0,
                                facility_tax: (_scCosts && _scCosts.facility_tax) || 0,
                                scc_surcharge: (_scCosts && _scCosts.scc_surcharge) || 0,
                                total_cost: (_scCosts && _scCosts.step_total) || _subMatCost || 0,
                            };
                            var _subCfg = Object.assign({}, parentConfig||{});
                            if (_subCfg.system_cost_index != null && _subCfg.system_cost_index < 1.0 && !_subCfg._sciFmt && !_subCfg.indices) {
                                _subCfg.system_cost_index = _subCfg.system_cost_index * 100;
                                _subCfg._sciFmt = 'pct';
                            }
                            var _rSub = {
                                id: order.id+'-sub-'+_rpid+'-'+depth,
                                order_number: order.order_number+'-'+String(_subCnt).padStart(3,'0'),
                                name: 'Sub '+String(_subCnt).padStart(3,'0')+' - '+(_subS.product_name||'Component'),
                                type: 'sub', parent_id: parentOrderId, _parentMatTypeId: _rpid,
                                items: [{blueprint_type_id:_rbpId,product_type_id:_rpid,product_name:_subS.product_name||'Component',runs:_subS.runs_needed||1,me:meLevel,te:teLevel}],
                                config: _subCfg,
                                build_cost: _subBC,
                                sub_orders: [], created_at: new Date().toISOString()
                            };
                            var _insIdx2 = _mainOrderIdx + _subCnt;
                            if (_insIdx2 > _productionOrders.length) _insIdx2 = _productionOrders.length;
                            _productionOrders.splice(_insIdx2, 0, _rSub);
                            if (_activeOrderIndex >= _insIdx2) _activeOrderIndex++;
                            if (_subS.sub_steps && _subS.sub_steps.length > 0) {
                                _createSubOrdersRecursive({steps: [{sub_steps: _subS.sub_steps}]}, _rSub.id, _matchedSC ? _matchedSC.sub_components : null, meLevel, teLevel, parentConfig, depth+1);
                            }
                        }
                    }
                }
                for (var _sii = 0; _sii < order.items.length; _sii++) {
                    var _siItem = order.items[_sii];
                    if (_siItem._buildStepsData) {
                        var _siSubComps = (_siItem._costs && _siItem._costs.items) ? _siItem._costs.items : null;
                        _createSubOrdersRecursive(_siItem._buildStepsData, order.id, _siSubComps, 10, 20, order.config, 1);
                    }
                }
                // ── Sub-Order Kosten per API berechnen + Propagation ──
                var _subFetchPromises = [];
                for (var _spi = 0; _spi < _productionOrders.length; _spi++) {
                    if (_productionOrders[_spi].parent_id === _mainOrderId) {
                        _subFetchPromises.push(_fetchBuildCostsForOrder(_productionOrders[_spi]));
                    }
                }
                await Promise.all(_subFetchPromises);
                // Propagation: Sub-Order Total → Parent Material (auch von Sub-Ordn zu Main)
                var _propagateTarget = order; // Default: the order being processed
                if (order.type === 'sub' && order.parent_id) {
                    // Bei Sub-Order Refresh: zum Parent propagieren
                    var _propParent = _productionOrders.filter(function(x) { return x.id === order.parent_id; });
                    if (_propParent.length > 0) _propagateTarget = _propParent[0];
                }
                console.log("[BP] Propagating Sub-Order costs... target="+_propagateTarget.name+" subs="+_productionOrders.filter(function(x){return x.type==='sub';}).length);
                for (var _ppi = 0; _ppi < _productionOrders.length; _ppi++) {
                    ...
                    if (_ppBC && _ppBC.total_cost > 0) {
                        ...
                        var _ppItemCosts = _ppo.items && _ppo.items[0] && _ppo.items[0]._costs;
                        if (_ppItemCosts && _ppItemCosts.total_cost) {
                            _ppTotal = _ppItemCosts.total_cost;
                        }
                    }
                    if (_ppTotal <= 0) continue;
                    var _ppRuns = _ppo.items && _ppo.items[0] ? (_ppo.items[0].runs || 1) : 1;
                    var _ppUnitPrice = _ppRuns > 0 ? _ppTotal / _ppRuns : 0;
                    console.log("[BP] SubOrder total="+formatIsk(_ppTotal)+" typeId="+_ppMatId+" runs="+_ppRuns+" unit="+formatIsk(_ppUnitPrice));
                    for (var _ppmi = 0; _ppmi < _propagateTarget.items.length; _ppmi++) {
                        var _ppItem = _propagateTarget.items[_ppmi];
                        ...
                        for (var _ppm = 0; _ppm < _ppItem.materials.length; _ppm++) {
                            ...
                            if (_ppMat.material_type_id === _ppMatId && _ppMat.decision === 'build') {
                                console.log("[BP] ✓ Propagated to material: "+(_ppMat.material_name||'?')+" total="+formatIsk(_ppTotal));
                                _ppMat._subOrderTotal = _ppTotal;
                                _ppMat.total_cost = _ppTotal;
                                _ppMat.unit_price = _ppUnitPrice;
                                break;
                            }
                        }
                    }
                }
                // Recalc mit den propagierten Sub-Order Kosten
                for (var _rci2 = 0; _rci2 < order.items.length; _rci2++) {
                    recalcOrderItem(order, _rci2);
                }
                // Gespeicherte Sub-Order ME/TE Änderungen wiederherstellen
                for (var _pppi = 0; _pppi < _productionOrders.length; _pppi++) {
                    var _pppo = _productionOrders[_pppi];
                    if (_pppo.type !== 'sub' || !_pppo.parent_id) continue;
                    var _pkey = _pppo._parentMatTypeId || (_pppo.items && _pppo.items[0] ? _pppo.items[0].product_type_id : null);
                    if (_pkey && _savedSubData[_pkey]) {
                        var _sd = _savedSubData[_pkey];
                        if (_pppo.items && _pppo.items[0]) {
                            _pppo.items[0].me = _sd.me;
                            _pppo.items[0].te = _sd.te;
                        }
                        if (_sd.config) {
                            _pppo.config = _sd.config;
                        }
                        if (_sd.materials && _pppo.items && _pppo.items[0] && _pppo.items[0].materials) {
                            for (var _smi2 = 0; _smi2 < _pppo.items[0].materials.length; _smi2++) {
                                var _smm2 = _pppo.items[0].materials[_smi2];
                                var _savedM = null;
                                for (var _smX = 0; _smX < _sd.materials.length; _smX++) {
                                    if (_sd.materials[_smX].material_type_id === _smm2.material_type_id) {
                                        _savedM = _sd.materials[_smX]; break;
                                    }
                                }
                                if (_savedM) {
                                    _smm2.decision = _savedM.decision;
                                    if (_savedM._priceMode) _smm2._priceMode = _savedM._priceMode;
                                }
                            }
                        }
                    }
    ...
    function renderOrders() {
        renderOrderSelector();
        renderConfigBar();
        var _isSub = _activeOrderIndex >= 0 && _productionOrders[_activeOrderIndex] && _productionOrders[_activeOrderIndex].type === 'sub';
        if (_isSub) {
            renderSubOrderDetail();
        } else {
            renderOrderDetail();
        }
        renderOrderCount();
    ...
    function renderOrderCount() {
        ...
        if (badge) {
            var _mainCount = _productionOrders.filter(function(o) { return o.type !== 'sub'; }).length;
            badge.textContent = _mainCount;
            badge.style.display = _mainCount > 0 ? "inline" : "none";
        }
    }
    ...
    function renderOrderSelector() {
        ...
        // Auto-select first main order if none active
        if (_activeOrderIndex < 0 || !_productionOrders[_activeOrderIndex]) {
            var _firstMain = _productionOrders.find(function(o) { return o.type !== 'sub'; });
            if (_firstMain) _activeOrderIndex = _productionOrders.indexOf(_firstMain);
        }
        var _curOrder = (_activeOrderIndex >= 0) ? _productionOrders[_activeOrderIndex] : null;
        var _mainOrderId = null;
        var _subOrders = [];
        if (_curOrder) {
            if (_curOrder.type === 'sub' && _curOrder.parent_id) {
                _mainOrderId = _curOrder.parent_id;
            } else {
                _mainOrderId = _curOrder.id;
            }
            _subOrders = _productionOrders.filter(function(o) { return o.parent_id === _mainOrderId; });
        }
        var _mainOrders = _productionOrders.filter(function(o) { return o.type !== 'sub'; });
        var _activeMain = _mainOrderId ? _productionOrders.find(function(o) { return o.id === _mainOrderId; }) : (_mainOrders.length > 0 ? _mainOrders[0] : null);
        var _mainIdx = _activeMain ? _productionOrders.indexOf(_activeMain) : -1;
        var html = '<div style="margin-bottom:4px;">';
        if (_activeMain) {
            html += '<div style="display:flex;gap:4px;flex-wrap:wrap;align-items:center;">';
            for (var _mi = 0; _mi < _mainOrders.length; _mi++) {
                var _mo = _mainOrders[_mi];
                var _miIdx = _productionOrders.indexOf(_mo);
                var _isMainActive = _miIdx === _mainIdx;
                var _scnt = _productionOrders.filter(function(x) { return x.parent_id === _mo.id; }).length;
                html += '<div class="bp-order-card' + (_isMainActive ? ' active' : '') + '"' +
                    ' onclick="BP.setActiveOrder(' + _miIdx + ')"' +
                    ' style="cursor:pointer;' + (_isMainActive ? 'background:rgba(138,180,248,0.12);border-color:var(--t-accent,#8ab4f8);' : '') + '">' +
                    '<div class="bp-order-card-name" style="font-size:0.72rem;"><i class="bi bi-file-earmark"></i> ' + escHtml(_mo.name) + '</div>' +
                    '<div class="bp-order-card-meta" style="font-size:0.6rem;">' +
                    (_scnt > 0 ? '<span class="badge bg-info" style="font-size:0.5rem;">' + _scnt + ' Sub</span> ' : '') +
                    '<span>' + (_mo.items ? _mo.items.length : 0) + ' items</span>' +
                    (_mo.build_cost ? '<span class="ms-2" style="color:var(--t-accent);font-weight:600;">' + formatIsk(_mo.build_cost.total_cost || 0) + '</span>' : '') +
                    '</div></div>';
            }
            html += '</div>';
        }
        ...
        if (_subOrders.length > 0) {
            html += '<div class="text-secondary mb-1" style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.05em;padding:0 4px;">' +
                '<i class="bi bi-diagram-2"></i> Sub-Orders (' + _subOrders.length + ')</div>';
            for (var _si = 0; _si < _subOrders.length; _si++) {
                var _so = _subOrders[_si];
                var _soIdx = _productionOrders.indexOf(_so);
                var _soAct = _soIdx === _activeOrderIndex;
                html += '<div class="bp-order-card' + (_soAct ? ' active' : '') + '"' +
                    ' onclick="BP.setActiveOrder(' + _soIdx + ')"' +
                    ' style="margin-left:12px;border-left:2px solid var(--t-accent,#8ab4f8);cursor:pointer;">' +
                    '<div class="bp-order-card-name" style="font-size:0.72rem;">' +
                    '<i class="bi bi-diagram-2" style="font-size:0.55rem;color:var(--t-accent);margin-right:2px;"></i> ' + escHtml(_so.name || 'Sub') + '</div>' +
                    '<div class="bp-order-card-meta">' +
                    '<span><i class="bi bi-box-seam"></i> ' + (_so.items ? _so.items.length : 0) + ' items</span>' +
                    '<span class="ms-2" style="color:var(--t-accent);font-weight:600;">' + formatIsk((_so.items && _so.items[0] && _so.items[0].build_cost && _so.items[0].build_cost.total_cost) || 0) + '</span>' +
                    '</div></div>';
            }
        } else if (_activeMain) {
            html += '<div class="text-secondary small text-center py-3">No Sub-Orders for this Main Order.<br>' +
                '<small>Send items from the Shopper cart to create production orders.</small></div>';
        }
        container.innerHTML = html;
    }

    async function applyConfigToSubOrders() {
        const order = _productionOrders[_activeOrderIndex];
        if (!order) return;
        var config = order.config;
        if (!config) return;
        var _subOrders = _productionOrders.filter(function(o) { return o.parent_id === order.id; });
        if (_subOrders.length === 0) return;
        // Config auf alle Sub-Orders kopieren
        for (var _asi = 0; _asi < _subOrders.length; _asi++) {
            _subOrders[_asi].config = JSON.parse(JSON.stringify(config));
        }
        // Sub-Orders mit neuer Config recalculieren
        var _subFetchPromises = [];
        for (var _spi2 = 0; _spi2 < _subOrders.length; _spi2++) {
            _subFetchPromises.push(_fetchBuildCostsForOrder(_subOrders[_spi2]));
        }
        await Promise.all(_subFetchPromises);
        // Main Order recalculieren (zieht neue Sub-Order Kosten via Propagation)
        var _activeIdx = _activeOrderIndex;
        await recalcCurrentOrder();
        _activeOrderIndex = _activeIdx;
        renderOrders();
    }

    /** Render Sub-Order Detail View */
    function renderSubOrderDetail() {
        ...
        if (_activeOrderIndex < 0 || _activeOrderIndex >= _productionOrders.length) return;
        const subOrder = _productionOrders[_activeOrderIndex];
        if (subOrder.type !== 'sub') return;
        detailEl.classList.add("active");
        placeholderEl.classList.remove("active");
        const nameEl = document.getElementById("bpOrderDetailName");
        if (nameEl) { nameEl.textContent = subOrder.name || 'Sub-Order'; nameEl.style.cursor = 'default'; nameEl.onclick = null; }
        const dateEl = document.getElementById("bpOrderDetailDate");
        if (dateEl) dateEl.textContent = subOrder.created_at ? new Date(subOrder.created_at).toLocaleDateString() : "";
        const metaEl = document.getElementById("bpOrderDetailMeta");
        if (metaEl && subOrder.parent_id) {
            var _parentOrder = _productionOrders.find(function(o) { return o.id === subOrder.parent_id; });
            if (_parentOrder) {
                metaEl.innerHTML = '<span class="text-info" style="cursor:pointer;" onclick="BP.setActiveOrder(' + _productionOrders.indexOf(_parentOrder) + ')">' +
                    '<i class="bi bi-arrow-up-circle"></i> ' + escHtml(_parentOrder.name) + '</span>' +
                    '<span class="ms-2 text-secondary"><i class="bi bi-diagram-2"></i> Sub-Order of ' + escHtml(_parentOrder.name) + '</span>';
            }
        } else if (metaEl) { metaEl.innerHTML = '<span class="text-secondary">Sub-Order</span>'; }
        const itemsContainer = document.getElementById("bpOrderItemsContainer");
        if (!itemsContainer) return;
        if (!subOrder.items || subOrder.items.length === 0) {
            itemsContainer.innerHTML = '<div class="text-secondary small text-center py-4">No items in this Sub-Order.</div>';
            const summaryEl = document.getElementById("bpOrderSummary");
            if (summaryEl) summaryEl.style.display = "none";
            const aggEl = document.getElementById("bpOrderAggMaterials");
            if (aggEl) aggEl.style.display = "none";
            const soEl = document.getElementById("bpOrderSubOrders");
    ...
    function renderOrderDetail() {
        ...
        for (let i = 0; i < order.items.length; i++) {
            ...
            if (expanded && hasMaterials) {
                ...
                html += '<div class="bp-order-mat-header">' +
                    ...
                    '<span class="bp-mat-col-price" style="color:#ffc107;font-weight:600;">Build</span>' +
                    '<span class="bp-mat-col-total">Total</span>' +
                    '<span class="bp-mat-col-action" style="font-size:0.5rem;color:#888;">BUILD/BUY</span>' +
                    '</div>';
                ...
                for (let mi = 0; mi < item.materials.length; mi++) {
                    ...
                    // Sell price per unit (override > API > cache > fallback)
                    var _effS = getEffectivePrice(m.material_type_id);
                    var sellPrice = (_effS && _effS.price != null) ? _effS.price : (m.sell_price_per_unit || (getPrice(m.material_type_id) || {}).sell_price_min || m.unit_price);
                    // Buy price per unit
                    var _effB = getEffectivePrice(m.material_type_id);
                    var buyPrice = (_effB && _effB.price != null) ? _effB.price : (m.buy_price_per_unit || (getPrice(m.material_type_id) || {}).buy_price_max || m.unit_price);
                    // Build price per unit (Sub-Order oder backend)
                    var buildPrice = (isBuild && m._subOrderTotal != null && m._subOrderTotal > 0 && m.total_quantity > 0)
                        ? (m._subOrderTotal / m.total_quantity) : null;
                    if (buildPrice == null && isBuild && m.total_cost != null && m.total_cost > 0 && m.total_quantity > 0)
                        buildPrice = m.total_cost / m.total_quantity;
                    // Cheapest: sell/buy/build vergleichen, günstigster grün
                    var _prices = [];
                    if (sellPrice > 0) _prices.push({k:'sell', v:sellPrice});
                    if (buyPrice > 0) _prices.push({k:'buy', v:buyPrice});
                    if (buildPrice > 0) _prices.push({k:'build', v:buildPrice});
                    var _minPrice = _prices.length > 0 ? Math.min.apply(null, _prices.map(function(p){return p.v;})) : null;
                    var _green = 'color:#28a745;font-weight:700;';
                    html += '<span class="bp-mat-col-sell" style="' + (sellPrice > 0 ? (_minPrice != null && sellPrice === _minPrice ? _green : '') : 'color:#555;') + '">' + (sellPrice > 0 ? formatIsk(sellPrice) : '-') + '</span>';
                    html += '<span class="bp-mat-col-buy" style="' + (buyPrice > 0 ? (_minPrice != null && buyPrice === _minPrice ? _green : '') : 'color:#555;') + '">' + (buyPrice > 0 ? formatIsk(buyPrice) : '-') + '</span>';
                    html += '<span class="bp-mat-col-price" style="font-weight:700;' + (buildPrice > 0 ? (_minPrice != null && buildPrice === _minPrice ? _green : 'color:#ffc107;') : 'color:#555;') + '">' + (buildPrice > 0 ? formatIsk(buildPrice) : '—') + '</span>';

                    // Effective price (via S/B toggle oder sell/buy/build)
                    var _effPrice = getEffectivePrice(m.material_type_id);
                    var unitPrice = (_effPrice && _effPrice.price != null) ? _effPrice.price : (sellPrice || buyPrice || m.unit_price || 0);
                    // Total cost — Sub-Order Total bevorzugen (von Propagation gesetzt)
                    var totalCost = 0;
                    if (isBuild && m._subOrderTotal != null && m._subOrderTotal > 0) {
                        totalCost = m._subOrderTotal;
                    } else if (isBuild && _hasOrdSub && _ordSubStep) {
                    ...
                    // Decision buttons: large BUILD / BUY toggles + small S/B price mode
                    var _priceMode = m._priceMode || 'sell';
                    var _priceModeSell = _priceMode === 'sell';
                    html += '<span class="bp-mat-col-action" style="display:flex;align-items:center;gap:3px;flex-wrap:nowrap;">' +
                        // BUILD button (always show — user decides what to build)
                        '<button class="bp-dec-btn ' + (isBuild ? 'active ' + (mIsReaction ? 'btn-dec-react' : 'btn-dec-build') : 'btn-dec-build inactive') + '"' +
                        ' onclick="event.stopPropagation();BP.toggleOrderMaterial(' + _activeOrderIndex + ',' + i + ',' + mi + ')"' +
                        ' title="' + (isBuild ? 'Currently BUILD - click to switch to BUY' : 'Switch to BUILD') + '">' +
                        (mIsReaction ? '<i class="bi bi-arrow-repeat"></i> REACT' : '<i class="bi bi-hammer"></i> BUILD') +
                        '</button>' +
                        // BUY button (always shown)
                        '<button class="bp-dec-btn ' + (isBuy ? 'active btn-dec-buy' : 'btn-dec-buy inactive') + '"' +
                        ' onclick="event.stopPropagation();BP.toggleOrderMaterial(' + _activeOrderIndex + ',' + i + ',' + mi + ',true)"' +
                        ' title="' + (isBuy ? 'Currently BUY - click to switch to BUILD' : 'Switch to BUY') + '">' +
                        '<i class="bi bi-cart"></i> BUY' +
                        '</button>' +
                        // Small S/B price mode toggle
                        '<span class="bp-price-mode-tog">' +
                        '<button class="' + (_priceModeSell ? 'active' : '') + '"' +
                        ' onclick="event.stopPropagation();BP.toggleMaterialPriceMode(' + _activeOrderIndex + ',' + i + ',' + mi + ')" title="Use sell price">S</button>' +
                        '<button class="' + (!_priceModeSell ? 'active' : '') + '"' +
                        ' onclick="event.stopPropagation();BP.toggleMaterialPriceMode(' + _activeOrderIndex + ',' + i + ',' + mi + ',true)" title="Use buy price">B</button>' +
                        '</span>' +
                        '</span>';
        ...
        // ── Summary ──
        console.log("[BP] renderOrderSummary called for:", order.name, "items:", order.items ? order.items.length : 0);
        if (order.items && order.items[0] && order.items[0].build_cost) {
            console.log("  build_cost.total_cost:", order.items[0].build_cost.total_cost);
            console.log("  build_cost.total_material_cost:", order.items[0].build_cost.total_material_cost);
        }
        renderOrderSummary();
    }
    ...
    function renderOrderAggregatedMaterials() {
        ...
        html += '<div class="bp-order-mat-header">' +
            ...
            '<span class="bp-mat-col-qty" style="text-align:right;font-weight:600;">Total</span>' +
            '<span class="bp-mat-col-sell">Sell</span>' +
            '<span class="bp-mat-col-buy">Buy</span>' +
            '<span class="bp-mat-col-price" style="color:#ffc107;font-weight:600;">Build</span>' +
            '<span class="bp-mat-col-total">Total</span>' +
            '<span style="font-size:0.6rem;color:#777;text-align:center;padding:0 2px;">Override</span>' +
            '<span class="bp-mat-col-action" style="font-size:0.55rem;color:#555;text-transform:uppercase;">BUILD/BUY</span>' +
            '</div>';
        ...
        for (var ki = 0; ki < ids.length; ki++) {
            ...
            // 3 prices (sell/buy/build) with cheapest green
            var _aggSellPr = e.sell_price || 0;
            var _aggBuyPr = e.buy_price || 0;
            var _aggBuildPr = (e.build_qty > 0 && e.build_cost > 0) ? (e.build_cost / e.build_qty) : 0;
            var _allPr = [];
            if (_aggSellPr > 0) _allPr.push(_aggSellPr);
            if (_aggBuyPr > 0) _allPr.push(_aggBuyPr);
            if (_aggBuildPr > 0) _allPr.push(_aggBuildPr);
            var _minP = _allPr.length > 0 ? Math.min.apply(null, _allPr) : null;
            var _gr = 'color:#28a745;font-weight:700;';
            html += '<span class="bp-mat-col-sell" style="' + (_aggSellPr > 0 && _minP && _aggSellPr === _minP ? _gr : '') + '">' + (_aggSellPr > 0 ? formatIsk(_aggSellPr) : '-') + '</span>';
            html += '<span class="bp-mat-col-buy" style="' + (_aggBuyPr > 0 && _minP && _aggBuyPr === _minP ? _gr : '') + '">' + (_aggBuyPr > 0 ? formatIsk(_aggBuyPr) : '-') + '</span>';
            html += '<span class="bp-mat-col-price" style="font-weight:700;' + (_aggBuildPr > 0 && _minP && _aggBuildPr === _minP ? _gr : 'color:#ffc107;') + '">' + (_aggBuildPr > 0 ? formatIsk(_aggBuildPr) : '—') + '</span>';
            ...
            html += '<span style="display:flex;align-items:center;gap:2px;">' +
                ...
                '</span>' +
                '<span class="bp-mat-col-action" style="display:flex;align-items:center;gap:3px;flex-wrap:nowrap;">' +
                ...
                (e.is_sub_material
                    ...
                    : (function(_catId) {
                        var _isBuildable = isBuildable({category_id: _catId});
                        var _aggIsBuild = e.build_qty >= e.buy_qty;
                        if (!_isBuildable) {
                            return '<span class="bp-dec-btn btn-dec-buy active" style="opacity:0.6;cursor:default;" title="Nicht baubar — immer kaufen">' +
                                '<i class="bi bi-cart"></i> BUY' +
                                '</span>';
                        }
                        return '<button class="bp-dec-btn ' + (_aggIsBuild ? 'active btn-dec-build' : 'btn-dec-build inactive') + '"' +
                            ' onclick="event.stopPropagation();BP.setAggOrderMaterialDecision(' + _tid + ',\'build\')" title="Alle Items auf BUILD setzen">' +
                            '<i class="bi bi-hammer"></i> BUILD' +
                            '</button>' +
                            '<button class="bp-dec-btn ' + (!_aggIsBuild ? 'active btn-dec-buy' : 'btn-dec-buy inactive') + '"' +
                            ' onclick="event.stopPropagation();BP.setAggOrderMaterialDecision(' + _tid + ',\'buy\')" title="Alle Items auf BUY setzen">' +
                            '<i class="bi bi-cart"></i> BUY' +
                            '</button>';
                    })(e.category_id)) +
                // Price mode S/B toggle
                '<span class="bp-price-mode-tog" style="margin-left:2px;">' +
                '<button class="' + (_aggPriceModeSell ? 'active' : '') + '"' +
                ' onclick="event.stopPropagation();BP.setAggOrderMaterialPriceMode(' + _tid + ',\'sell\')" title="Alle auf SELL-Preis">S</button>' +
                '<button class="' + (!_aggPriceModeSell ? 'active' : '') + '"' +
                ' onclick="event.stopPropagation();BP.setAggOrderMaterialPriceMode(' + _tid + ',\'buy\')" title="Alle auf BUY-Preis">B</button>' +
                '</span>' +
                '</span>';
            html += '</div>';
        }
    ...
    function toggleOrderMaterial(orderIndex, itemIndex, materialIndex, forceBuy) {
        ...
        // If this is a sub-order, propagate the new total to the parent
        if (order.type === 'sub' && order.parent_id) {
            _propagateSubOrderCost(order);
        }
    ...
    /** Propagate Sub-Order cost to parent material */
    function _propagateSubOrderCost(subOrder) {
        if (!subOrder.parent_id) return;
        var _parentOrder = _productionOrders.find(function(o) { return o.id === subOrder.parent_id; });
        if (!_parentOrder) return;
        var _ppMatId = subOrder._parentMatTypeId;
        if (!_ppMatId) return;
        var _ppTotal = subOrder.items && subOrder.items[0] && subOrder.items[0].build_cost && subOrder.items[0].build_cost.total_cost;
        if (!_ppTotal || _ppTotal <= 0) return;
        for (var _ppmi = 0; _ppmi < _parentOrder.items.length; _ppmi++) {
            var _ppItem = _parentOrder.items[_ppmi];
            if (!_ppItem.materials) continue;
            for (var _ppm = 0; _ppm < _ppItem.materials.length; _ppm++) {
                var _ppMat = _ppItem.materials[_ppm];
                if (_ppMat.material_type_id === _ppMatId && _ppMat.decision === 'build') {
                    _ppMat._subOrderTotal = _ppTotal;
                    _ppMat.total_cost = _ppTotal;
                    var _ppRuns = subOrder.items && subOrder.items[0] ? (subOrder.items[0].runs || 1) : 1;
                    _ppMat.unit_price = _ppRuns > 0 ? _ppTotal / _ppRuns : 0;
                    recalcOrderItem(_parentOrder, _ppmi);
                    return;
                }
            }
        }
    ...
    function recalcOrderItem(order, itemIndex) {
        ...
        for (const m of item.materials) {
            ...
            if (m.decision === 'build') {
                // Sub-Order Total bevorzugen (wurde von Propagation gesetzt)
                if (m._subOrderTotal != null && m._subOrderTotal > 0) {
                    buildTotal += m._subOrderTotal;
                    buildQty += m.total_quantity || 0;
                    continue;
                }
        ...
        // total_cost NICHT überschreiben — API-Wert beibehalten (der facility+job korrekt enthält)
        // recalcOrderItem überschreibt nur die breakdown-Felder, nicht den Gesamtwert
        if (!item.build_cost.total_cost || item.build_cost.total_cost <= 0) {
    ...
    function renderOrderSummary() {
        ...
        for (const item of order.items) {
            if (item.build_cost) {
                ...
                // Material cost — aus tatsächlichen Material-Daten (inkl. Override+Sub-Order)
                if (item.materials) {
                    var _matSum = 0;
                    for (var _mmi = 0; _mmi < item.materials.length; _mmi++) {
                        var _mm = item.materials[_mmi];
                        if (_mm.decision === 'build' && _mm._subOrderTotal != null && _mm._subOrderTotal > 0) {
                            _matSum += _mm._subOrderTotal;
                        } else if (_mm.decision === 'build' && _mm.total_cost != null && _mm.total_cost > 0) {
                            _matSum += _mm.total_cost;
                        } else {
                            var _mmEff = getEffectivePrice(_mm.material_type_id);
                            var _mmPr = (_mmEff && _mmEff.price != null) ? _mmEff.price : (_mm.unit_price || 0);
                            _matSum += _mmPr * (_mm.total_quantity || 0);
                        }
                    }
                    totalMaterialCost += _matSum;
                } else if (item.build_cost) {
        ...
        // Grand Total — API-Wert pro Item bevorzugen, sonst recalculated
        var grandTotal = 0;
        for (var _gti = 0; _gti < order.items.length; _gti++) {
            if (order.items[_gti].build_cost && order.items[_gti].build_cost.total_cost > 0) {
                grandTotal += order.items[_gti].build_cost.total_cost;
            }
        }
        if (grandTotal <= 0) {
            grandTotal = totalMaterialCost + totalJobCost + totalBpcCost;
        }
        console.log("[BP] Summary: mat="+formatIsk(totalMaterialCost)+" job="+formatIsk(totalJobCost)+" bpc="+formatIsk(totalBpcCost)+" grand="+formatIsk(grandTotal));
    ...
    function renderPriceOverrides() {
        ...
        // ── Add materials (auch aus Sub-Orders) ──
        var _allOrders = _productionOrders.filter(function(o) { return o.parent_id === order.id || o.id === order.id; });
        _allOrders.push(order);
        for (var oi = 0; oi < _allOrders.length; oi++) {
            var _ao = _allOrders[oi];
            for (var i = 0; i < (_ao.items || []).length; i++) {
                var item = _ao.items[i];
            if (!item.materials) continue;
            for (var mi = 0; mi < item.materials.length; mi++) {
                var m = item.materials[mi];
     ...
     */
    async function recalcOrderFromCache() {
        var order = _productionOrders[_activeOrderIndex];
        ...
        // ── Sub-Order Propagation ──
        var _subs = _productionOrders.filter(function(o) { return o.parent_id === order.id; });
        for (var _psi = 0; _psi < _subs.length; _psi++) {
            var _so = _subs[_psi];
            if (!_so.items || !_so.items[0]) continue;
            await recalcOrderItem(_so, 0);
            // ── Override Sub-Order Material Prices mit getEffectivePrice ──
            var _soItem = _so.items[0];
            if (_soItem.materials) {
                var _soMatSum = 0;
                for (var _somi = 0; _somi < _soItem.materials.length; _somi++) {
                    var _som = _soItem.materials[_somi];
                    var _effP = getEffectivePrice(_som.material_type_id);
                    if (_effP && _effP.price != null) {
                        _som.unit_price = _effP.price;
                        _som.total_cost = _effP.price * (_som.total_quantity || 0);
                    }
                    _soMatSum += _som.total_cost || 0;
                }
                // Update sub-order build_cost total mit override-preisen
                if (_soItem.build_cost) {
                    var _origTotal = _soItem.build_cost.total_cost || 0;
                    var _origMatCost = _soItem.build_cost.total_material_cost || 0;
                    _soItem.build_cost.total_material_cost = _soMatSum;
                    // Facility costs bleiben gleich, nur Material wird ersetzt
                    _soItem.build_cost.total_cost = _soMatSum + (_origTotal - _origMatCost);
                }
            }
            var _ppMatId = _so._parentMatTypeId || (_soItem.product_type_id);
            var _ppTotal = (_soItem.build_cost && _soItem.build_cost.total_cost > 0) ? _soItem.build_cost.total_cost : 0;
            if (_ppTotal <= 0) continue;
            var _ppRuns = _soItem.runs || 1;
            var _ppUnitPrice = _ppRuns > 0 ? _ppTotal / _ppRuns : 0;
            for (var _ppmi = 0; _ppmi < order.items.length; _ppmi++) {
                var _ppItem = order.items[_ppmi];
                if (!_ppItem.materials) continue;
                for (var _ppm = 0; _ppm < _ppItem.materials.length; _ppm++) {
                    var _ppMat = _ppItem.materials[_ppm];
                    if (_ppMat.material_type_id === _ppMatId && _ppMat.decision === 'build') {
        ...
        // Recalc main order again WITH sub-order propagation
        for (var i = 0; i < order.items.length; i++) {
            recalcOrderItem(order, i);
        }
    ...
    function exportBuyOrder() {
        ...
        async function generateBuyList() {
            ...
            // Aggregate buy materials: MAIN order dictates what to buy.
            // Items marked BUY in main → only that item itself.
            // Items marked BUILD in main → decompose to sub-order's BUY materials.
            const buyMap = {}; // material_type_id -> { name, qty, total_cost }
            let hasCostData = false;

            // 1. MAIN order: BUY items direkt übernehmen, BUILD items via Sub-Orders
            var _subOrders = _productionOrders.filter(function(o) { return o.parent_id === order.id; });
            for (var _ii = 0; _ii < (order.items || []).length; _ii++) {
                var _item = order.items[_ii];
                if (!_item.materials) continue;
                for (var _mi2 = 0; _mi2 < _item.materials.length; _mi2++) {
                    var _m = _item.materials[_mi2];
                    var key = _m.material_type_id;
                    if (_m.decision === 'buy') {
                        // Direkt kaufen → NUR das Item selbst
                        var _effP = getEffectivePrice(key);
                        var _effPrice = (_effP && _effP.price != null) ? _effP.price : (_m.unit_price || 0);
                        if (!buyMap[key]) {
                            buyMap[key] = {
                                name: _m.material_name || ("Material " + key),
                                type_id: key,
                                qty: 0,
                                unit_price: _effPrice,
                                total_cost: 0
                            };
                        }
                        buyMap[key].qty += _m.total_quantity;
                        var cost = _effPrice * _m.total_quantity;
                        buyMap[key].total_cost += cost;
                        if (_effPrice > 0) hasCostData = true;
                    } else if (_m.decision === 'build') {
                        // Selbst bauen → Sub-Order's BUY-Materialien sammeln
                        for (var _soi = 0; _soi < _subOrders.length; _soi++) {
                            var _so = _subOrders[_soi];
                            if (_so._parentMatTypeId !== key) continue;
                            for (var _smi = 0; _smi < (_so.items || []).length; _smi++) {
                                var _soItem = _so.items[_smi];
                                if (!_soItem.materials) continue;
                                for (var _smmi = 0; _smmi < _soItem.materials.length; _smmi++) {
                                    var _sm = _soItem.materials[_smmi];
                                    if (_sm.decision !== 'buy') continue;
                                    var _sk = _sm.material_type_id;
                                    var _effP2 = getEffectivePrice(_sk);
                                    var _effPrice2 = (_effP2 && _effP2.price != null) ? _effP2.price : (_sm.unit_price || 0);
                                    if (!buyMap[_sk]) {
                                        buyMap[_sk] = {
                                            name: _sm.material_name || ("Material " + _sk),
                                            type_id: _sk,
                                            qty: 0,
                                            unit_price: _effPrice2,
                                            total_cost: 0
                                        };
                                    }
                                    buyMap[_sk].qty += _sm.total_quantity;
                                    var cost2 = _effPrice2 * _sm.total_quantity;
                                    buyMap[_sk].total_cost += cost2;
                                    if (_effPrice2 > 0) hasCostData = true;
                                }
                            }
                        }
                    }
            ...
            // 2. Sub-Sub-Orders nur dann durchgehen, wenn deren Parent-Sub-Order
            //    zu einem BUILD-Material in der Main Order gehört
            for (var _ssi = 0; _ssi < _subOrders.length; _ssi++) {
                var _parentMatId = _subOrders[_ssi]._parentMatTypeId;
                // Prüfe ob das Parent-Material noch BUILD ist
                var _stillBuild = false;
                for (var _chk = 0; _chk < (order.items || []).length; _chk++) {
                    var _chkItem = order.items[_chk];
                    if (_chkItem.materials) {
                        for (var _chkm = 0; _chkm < _chkItem.materials.length; _chkm++) {
                            if (_chkItem.materials[_chkm].material_type_id === _parentMatId &&
                                _chkItem.materials[_chkm].decision === 'build') {
                                _stillBuild = true;
                                break;
                            }
                        }
                        if (_stillBuild) break;
                    }
                }
                if (!_stillBuild) continue;
                // Sub-Sub-Orders dieser Sub-Order durchgehen
                var _subSubs = _productionOrders.filter(function(o) { return o.parent_id === _subOrders[_ssi].id; });
                for (var _sssi = 0; _sssi < _subSubs.length; _sssi++) {
                    var _sso = _subSubs[_sssi];
                    for (var _ssii = 0; _ssii < (_sso.items || []).length; _ssii++) {
                        var _ssItem = _sso.items[_ssii];
                        if (!_ssItem.materials) continue;
                        for (var _ssmi = 0; _ssmi < _ssItem.materials.length; _ssmi++) {
                            var _ssm = _ssItem.materials[_ssmi];
                            if (_ssm.decision !== 'buy') continue;
                            var _ssk = _ssm.material_type_id;
                            var _effP3 = getEffectivePrice(_ssk);
                            var _effPrice3 = (_effP3 && _effP3.price != null) ? _effP3.price : (_ssm.unit_price || 0);
                            if (!buyMap[_ssk]) {
                                buyMap[_ssk] = {
                                    name: _ssm.material_name || ("Material " + _ssk),
                                    type_id: _ssk,
                                    qty: 0,
                                    unit_price: _effPrice3,
                                    total_cost: 0
                                };
                            }
                            buyMap[_ssk].qty += _ssm.total_quantity;
                            var cost3 = _effPrice3 * _ssm.total_quantity;
                            buyMap[_ssk].total_cost += cost3;
                            if (_effPrice3 > 0) hasCostData = true;
                        }
                    }
                }
            }

            var entries = Object.values(buyMap).sort(function (a, b) {
            ...
            if (entries.length === 0) {
                ...
                window.buyListCopyClipboard = buyListCopyClipboard;
                window.buyListExportCsv = buyListExportCsv;
                window.buyListExportText = buyListExportText;
            ...
            // Buy-List Helfer global für Modal-Buttons
            window.buyListCopyClipboard = buyListCopyClipboard;
            window.buyListExportCsv = buyListExportCsv;
            window.buyListExportText = buyListExportText;
        ...
        function buyListCopyClipboard() {
            ...
            // Ersetze Modal-Body mit Textarea + Select-All-Button
            var modalBody = document.getElementById("bpBuyListBody");
            if (!modalBody) { alert("Modal body missing"); return; }
            modalBody.innerHTML = '<div class="text-info small mb-2">' +
                '<i class="bi bi-info-circle"></i> Text markiert — drücke Strg+C zum Kopieren:</div>' +
                '<textarea id="bpBuyListTextarea" readonly class="form-control form-control-sm bg-dark text-light border-secondary" ' +
                'style="font-size:0.72rem;height:250px;font-family:monospace;white-space:pre;overflow:auto;">' +
                escHtml(text) + '</textarea>' +
                '<button class="btn btn-sm btn-outline-info mt-2 w-100" onclick="var t=document.getElementById(\'bpBuyListTextarea\');t.select();document.execCommand(\'copy\');this.innerHTML=\'<i class=\\\'bi bi-check-lg\\\'></i> Kopiert!\';setTimeout(function(){this.innerHTML=\'<i class=\\\'bi bi-clipboard\\\'></i> Erneut kopieren\';}.bind(this),2000);">' +
                '<i class="bi bi-clipboard"></i> In Zwischenablage kopieren</button>';
            // Auto-select nach kurzer Verzögerung
            setTimeout(function() {
                var ta = document.getElementById("bpBuyListTextarea");
        ...
        function buyListExportText() {
            ...
            // Final fallback: show textarea in modal
            var ta = document.createElement("textarea");
            ta.value = text;
            ta.style.width = "100%";
            ...
            var modalBody = document.getElementById("bpBuyListBody");
            if (modalBody) {
                modalBody.innerHTML = '<div class="text-info small mb-1">Manuell kopieren (Strg+A, Strg+C):</div>';
                modalBody.appendChild(ta);
            }
            var btn = document.querySelector('#bpBuyListModal .modal-footer .btn-outline-warning');
            if (btn) {
                var orig = btn.innerHTML;
                btn.innerHTML = '<i class="bi bi-info-circle"></i> Mark & Copy';
                setTimeout(function () { btn.innerHTML = orig; }, 3000);
            }
        }

        // Buy-List Helfer global (WIRD NOCHMAL AUF IIFE-EBENE GESETZT, siehe unten)
    ...
    /** Debug: full order snapshot — captures ALL display values vs stored data */
    function debugPricingDiagnostic() {
        var lines = [];
        lines.push("=== FULL ORDER SNAPSHOT ===");
        lines.push("Timestamp: " + new Date().toISOString());
        lines.push("Active Index: " + _activeOrderIndex);
        for (var oi = 0; oi < _productionOrders.length; oi++) {
            var o = _productionOrders[oi];
            lines.push("═══════════════════════════════");
            lines.push("ORDER " + oi + ": " + (o.name || '?') + " | type=" + (o.type || 'main') + " | id=" + o.id + (o.parent_id ? " | parent=" + o.parent_id : ""));
            lines.push("  Config SCI=" + (o.config ? o.config.system_cost_index : 'N/A') + " Tax=" + (o.config ? o.config.tax_rate : 'N/A') + " Fac=" + (o.config ? o.config.facility_type : 'N/A'));
            if (o.items) {
                for (var ii = 0; ii < o.items.length; ii++) {
                    var item = o.items[ii];
                    lines.push("  ── ITEM " + ii + ": " + (item.product_name || item.blueprint_type_id || '?'));
                    lines.push("    runs=" + (item.runs || 1) + " me=" + (item.me || 0) + " te=" + (item.te || 0));
                    if (item.build_cost) {
                        lines.push("    build_cost.total_cost=" + formatIsk(item.build_cost.total_cost || 0));
                        lines.push("    build_cost.total_material_cost=" + formatIsk(item.build_cost.total_material_cost || 0));
                        lines.push("    build_cost.facility_cost=" + formatIsk(item.build_cost.facility_cost || 0));
                        lines.push("    build_cost.job_cost=" + formatIsk(item.build_cost.job_cost || 0));
                        lines.push("    build_cost.cost_per_unit=" + formatIsk(item.build_cost.cost_per_unit || 0));
                        lines.push("    build_cost.system_cost_amount=" + formatIsk(item.build_cost.system_cost_amount || 0));
                        lines.push("    build_cost.facility_tax=" + formatIsk(item.build_cost.facility_tax || 0));
                        lines.push("    build_cost.scc_surcharge=" + formatIsk(item.build_cost.scc_surcharge || 0));
                        lines.push("    build_cost.total_job_cost=" + formatIsk(item.build_cost.total_job_cost || 0));
                        lines.push("    build_cost.build_material_total=" + formatIsk(item.build_cost.build_material_total || 0));
                        lines.push("    build_cost.buy_material_total=" + formatIsk(item.build_cost.buy_material_total || 0));
                        lines.push("    build_cost.installation_cost=" + formatIsk(item.build_cost.installation_cost || 0));
                    }
                    if (item.materials) {
                        for (var mi = 0; mi < item.materials.length; mi++) {
                            var m = item.materials[mi];
                            lines.push("    ─ MAT " + mi + ": " + (m.material_name || '?') + " (typeId=" + m.material_type_id + ")");
                            lines.push("      decision=" + (m.decision || '?') + " qty=" + (m.total_quantity || 0));
                            lines.push("      unit_price=" + (m.unit_price != null ? formatIsk(m.unit_price) : 'N/A'));
                            lines.push("      total_cost=" + (m.total_cost != null ? formatIsk(m.total_cost) : 'N/A'));
                            lines.push("      _subOrderTotal=" + (m._subOrderTotal != null ? formatIsk(m._subOrderTotal) : 'N/A'));
                            lines.push("      sell_price_per_unit=" + (m.sell_price_per_unit != null ? formatIsk(m.sell_price_per_unit) : 'N/A'));
                            lines.push("      buy_price_per_unit=" + (m.buy_price_per_unit != null ? formatIsk(m.buy_price_per_unit) : 'N/A'));
                            lines.push("      _priceMode=" + (m._priceMode || '?'));
                            lines.push("      is_reaction=" + (m.is_reaction ? 'true' : 'false'));
                            lines.push("      is_buildable=" + (m.is_buildable ? 'true' : 'false'));
                        }
                    }
                }
            }
            // For sub-orders, also show what the PARENT's material says
            if (o.type === 'sub' && o.parent_id) {
                var parentOrder = _productionOrders.find(function(p) { return p.id === o.parent_id; });
                var subTotal = o.items && o.items[0] && o.items[0].build_cost && o.items[0].build_cost.total_cost;
                var matId = o._parentMatTypeId;
                lines.push("  ▶ PARENT COMPARE (typeId=" + matId + ")");
                lines.push("    Sub-Order build_cost.total_cost: " + (subTotal ? formatIsk(subTotal) : 'N/A'));
                if (parentOrder) {
                    for (var pi = 0; pi < parentOrder.items.length; pi++) {
                        var pItem = parentOrder.items[pi];
                        if (!pItem.materials) continue;
                        for (var pmi = 0; pmi < pItem.materials.length; pmi++) {
                            var pm = pItem.materials[pmi];
                            if (pm.material_type_id === matId) {
                                lines.push("    Parent material._subOrderTotal: " + (pm._subOrderTotal != null ? formatIsk(pm._subOrderTotal) : 'N/A'));
                                lines.push("    Parent material.total_cost: " + (pm.total_cost != null ? formatIsk(pm.total_cost) : 'N/A'));
                                lines.push("    Parent material.unit_price: " + (pm.unit_price != null ? formatIsk(pm.unit_price) : 'N/A'));
                                lines.push("    Parent material.decision: " + (pm.decision || '?'));
                                lines.push("    Parent item.build_cost.total_cost: " + formatIsk(pItem.build_cost ? pItem.build_cost.total_cost : 0));
                                lines.push("    Parent item.build_cost.cost_per_unit: " + formatIsk(pItem.build_cost ? pItem.build_cost.cost_per_unit : 0));
                                lines.push("    MATCH: " + (subTotal === pm._subOrderTotal ? '✓' : '✗'));
                                break;
                            }
                        }
                    }
        ...
        console.log(text);
        // Show in modal
        var existing = document.getElementById("bpDiagModal");
        if (existing) existing.remove();
        var modal = document.createElement("div");
        modal.id = "bpDiagModal";
        modal.style.cssText = "position:fixed;top:5%;left:5%;width:90%;height:90%;z-index:9999;background:#1a1a2e;border:1px solid rgba(255,255,255,0.2);border-radius:8px;padding:16px;overflow:auto;box-shadow:0 8px 32px rgba(0,0,0,0.7);";
        modal.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
            '<h5 style="margin:0;color:#ffc107;">Full Order Snapshot</h5>' +
            '<div>' +
            '<button onclick="navigator.clipboard.writeText(document.querySelector(\'#bpDiagModal pre\').textContent);this.innerHTML=\'✅ Copied!\'" ' +
            'style="padding:4px 12px;background:#0d6efd;color:#fff;border:none;border-radius:4px;cursor:pointer;margin-right:4px;">📋 Copy</button>' +
            '<button onclick="this.parentElement.parentElement.parentElement.remove()" style="background:none;border:none;color:#888;font-size:1.2rem;cursor:pointer;">✕</button></div></div>' +
            '<pre style="font-size:0.62rem;color:#ccc;white-space:pre-wrap;margin:0;font-family:monospace;">' + escHtml(text) + '</pre>';
    ...
    window.BP = {
        ...
        applyConfigToSubOrders: applyConfigToSubOrders,
        debugPricingDiagnostic: debugPricingDiagnostic,
        diagnosticCompare: _diagnosticCompare,
