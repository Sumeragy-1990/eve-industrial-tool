    ...
    for item in body.cart_items:
        ...
        for m in mats:
            ...
            if m_is_reaction:
                ...
                # Apply EVE-correct ME formula: waste = baseQty * 0.1 / (1 + ME)
                me_factor = 1.0 - (0.1 / (1.0 + item.me))
                adjusted = max(1, math.ceil(base_qty * max(0.01, me_factor) * (1.0 - (getattr(item, '_rig_bonus', 0.0) or 0.0))))
    ...
    async def _fetch_materials(bp_type_id: int, me_level: int, te_level: int,
        ...
        for row in rows:
            ...
            if is_reaction:
                ...
                adjusted = base_qty
                if base_qty > 0:
                    me_factor = 1.0 - (0.1 / (1.0 + me_level))
                    adjusted = max(1, math.ceil(base_qty * max(0.01, me_factor) * (1.0 - (getattr(item, '_rig_bonus', 0.0) or 0.0))))
