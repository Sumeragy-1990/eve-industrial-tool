...
async def calculate_build_cost(
    ...
    # Deshalb: ALLE Ship-Rigs (_large_/ _medium_/ _small_) matchen category_id=6 (Ship).
    # Perfekte Filterung erfordert Import von dgmTypeAttributes.
    _RIG_CAT_MAP = {
        "ship": [6],            # category_id 6 = Ship (alle Größen)
        "adv_component": [873], # Advanced Component
        "component": [7],       # Component
    }
    # Hilfsfunktion: Prüft ob ein Rig-Name auf die Kategorie passt
    def _rig_matches(rig_name, prod_cat):
        for key, cats in _RIG_CAT_MAP.items():
            if key in rig_name.lower() and prod_cat in cats:
                return True
        return False
    # Lookup product category for each item
    for item in body.cart_items:
        prod_cat = None
        if item.product_type_id:
            cat_sql = text("SELECT category_id FROM sde_items WHERE type_id = :tid")
            cat_res = await db.execute(cat_sql, {"tid": item.product_type_id})
            cat_row = cat_res.fetchone()
            if cat_row:
                prod_cat = cat_row[0]
        # Filter rigs that match this item's category
        matched_bonus = 0.0
        for r in rig_names:
            r_clean = r.strip()
            ...
            if rig_bonus <= 0:
                continue
            if prod_cat is None or _rig_matches(r_clean, prod_cat):
