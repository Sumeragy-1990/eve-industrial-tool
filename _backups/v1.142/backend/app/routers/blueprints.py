    ...
    for item in body.cart_items:
        ...
        for m in mats:
            ...
            if m_is_reaction:
                ...
                # EVE-korrekte Formel: base + waste, Rig/Structure reduziert NUR den Waste
                waste_rate = 0.1 / (1.0 + item.me)
                total_bonus = (getattr(item, '_rig_bonus', 0.0) or 0.0)
                effective_waste = waste_rate * max(0.0, 1.0 - total_bonus)
                adjusted = max(1, math.ceil(base_qty * (1.0 + effective_waste)))
